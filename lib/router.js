...
methods.forEach(function (method) {
  Router.prototype[method] = function (name, path, middleware) {
    ...
    // for versioning
    middleware = middleware.map(function(fn){
      if(typeof fn === 'object') {
        return function* (){
          var handler = fn[this[opts.versionIdentifier]] || fn.default
          if(handler) {
            yield handler;
          }else {
            throw new Error("neither [" + this[opts.versionIdentifier] + "] nor [default] handler found");
          }
        }
      }else{
        return fn;
